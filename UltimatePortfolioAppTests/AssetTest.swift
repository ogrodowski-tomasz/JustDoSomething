...
class AssetTest: XCTestCase {
    ...
    ///
    /// Loading color strings into a SwiftUI Color Struct will always succeed.
    /// If the color name is just missing it will still work and you'lll get an error log
    /// in the debug output which isn't very useful.
    /// We cannot test that, becuase it'll always work.
    /// Instead, we want to load color using UIKit, because UIColor(named: ) will turn the optional UIColor
    /// which can be verified if it contain a value or not (seeing if it passed or failed).
